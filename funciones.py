...
def buscar_por_numero_documento(df, numero_documento):
    try:
        numero_documento = str(numero_documento)
        registros = df[df['Numero_Documento_Paciente'].astype(str) == numero_documento]
        if registros.empty:
...
def buscar_por_numero_documento_personal(df, numero_documento):
    try:
        numero_documento = str(numero_documento)
        registros = df[df['Numero_Documento_Personal'].astype(str) == numero_documento]
        if registros.empty:
            messagebox.showerror("Error", f"No se encontró ningún registro con el Número de Documento {numero_documento}")
            return
